...
class FlexMatch:
    ...
    def train(self, args, logger=None):
        ...
        D1 = torch.zeros(30,30).cuda(args.gpu)
        D1[0][1]=1
        D1[0][2]=1 
        D1[1][3]=1
        D1[1][4]=1
        ...
        D1[9][28]=1
        D1[9][29]=1        
        I = torch.eye(30).cuda(args.gpu)
        A = torch.inverse(I - D1)
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with torch.no_grad():
                self.model.embedding.weight.data = torch.transpose(project_simplex(torch.transpose(self.model.embedding.weight, 0, 1)), 0, 1)
