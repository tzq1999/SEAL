...
class Reconstructioneval():
    ...
    def embedding_distances(self,x):
        a=[np.linalg.norm(self.embedding[x])**2 for i in range(self.embedding.shape[0])]
        b=[np.linalg.norm(self.embedding[i])**2 for i in range(self.embedding.shape[0])] 
        c=[np.linalg.norm(self.embedding[x]-self.embedding[i])**2 for i in range(self.embedding.shape[0])]
        a=np.array(a)
        b=np.array(b)
        c=np.array(c)
        return np.arccosh(1+c*2/((1-a)*(1-b)))
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("/home/zqtan/hyper embed/epoch_30000"))
    embedding=model.embedding.weight.data.numpy()   #get embedding array from model 

    #generate_report(model,data)
    linkpreeval=LinkPredictionEvaluation("/home/zqtan/hyper embed/mammal_closure_part1.tsv","/home/zqtan/hyper embed/mammal_closure_part2.tsv",embedding,data.item2id)
