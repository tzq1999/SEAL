...
class Reconstructioneval():
    ...
    def embedding_distances(self,x):
        a=[np.linalg.norm(self.embedding[x])**2 for i in range(self.embedding.shape[0]-1)]
        b=[np.linalg.norm(self.embedding[i])**2 for i in range(self.embedding.shape[0]) if i !=x ] 
        c=[np.linalg.norm(self.embedding[x]-self.embedding[i])**2 for i in range(self.embedding.shape[0]) if i !=x ]
        a=np.array(a)
        b=np.array(b)
        c=np.array(c)
...
class LinkPredictionEvaluation():
    ...
    def evaluate_mean_rank_and_map(self, max_n=None):
        ...
        for i, item in enumerate(self.items):
            ...
            item_distances = self.embedding_distances(item_term)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("/home/zqtan/hyper embed/epoch_9000"))
